...
// ── DELETE ACCOUNT TAB ──
function DeleteAccountTab(){const{user,logout}=useAuth();const navigate=useNavigate();const[confirm,setConfirm]=useState("");const[loading,setLoading]=useState(false);const[phase,setPhase]=useState("idle");const[err,setErr]=useState("");const handleDelete=async()=>{if(confirm!=="DELETE"){setErr("Please type DELETE exactly to confirm.");return;}setLoading(true);setErr("");try{if(user?.uid){await fetch("https://beme-market-1.onrender.com/api/sellers/delete-account",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({uid:user.uid})}).catch(()=>{});await deleteDoc(doc(db,"users",user.uid)).catch(()=>{});}await logout().catch(()=>{});setPhase("done");setTimeout(()=>navigate("/"),2500);}catch(e){setErr(e.message||"Failed to delete account. Please contact support.");setLoading(false);}};
