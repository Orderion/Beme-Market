  ...
  return (
    <div style={{ background:"#fff" }}>
      <div className="sd-page-head">
        ...
        <button style={{ padding:"10px 20px", background:"#111", color:"#fff", border:"none", borderRadius:10, fontSize:13, fontWeight:800, cursor:"pointer" }} onClick={() => setShowForm(true)}>
          + Request Withdrawal
        </button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(180px, 1fr))", gap:14, marginBottom:14 }}>
        {[
          ...
          { label:"Total Withdrawn",   value:`GHS ${completedTotal.toFixed(2)}`, note:"All time completed",        color:"#111" },
        ].map((c) => (
          <div key={c.label} className="sd-stat-card" style={{ background:"#fff", border:"1px solid rgba(0,0,0,0.08)" }}>
      ...
      <div style={{ display:"flex", gap:10, padding:"12px 16px", background:"rgba(124,58,237,0.04)", borderRadius:10, border:"1px solid rgba(124,58,237,0.12)", marginBottom:14 }}>
        ...
        <div style={{ fontSize:13, color:"#6B7280", lineHeight:1.5 }}>
          <strong>Payout Policy:</strong> Minimum withdrawal is GHS {MIN_WITHDRAWAL}. Payouts are processed within 1–3 business days. Ensure your details are accurate.
        </div>
      </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
          <div style={{ background:"#fff", borderRadius:16, padding:28, width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
              <h3 style={{ fontSize:17, fontWeight:800, color:"#111" }}>Request Withdrawal</h3>
              <button onClick={() => setShowForm(false)} style={{ background:"none", border:"none", cursor:"pointer", color:"#9CA3AF", fontSize:20 }}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="sd-form-group">
                ...
                <div style={{ display:"flex", gap:10 }}>
                  {Object.entries(PAYOUT_METHODS).map(([key, m]) => (
                    <label key={key} style={{ flex:1, padding:"10px 14px", borderRadius:8, border:`2px solid ${form.method===key?"#111":"rgba(0,0,0,0.1)"}`, cursor:"pointer", display:"flex", alignItems:"center", gap:8, background:form.method===key?"rgba(0,0,0,0.03)":"transparent" }}>
                      <input type="radio" name="method" value={key} checked={form.method===key} onChange={upd("method")} style={{ display:"none" }}/>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={form.method===key?"#111":"#9CA3AF"} strokeWidth="1.8" strokeLinecap="round">
                        {key==="momo" ? <><rect x="5" y="2" width="14" height="20" rx="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></> : <><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="21" x2="9" y2="9"/></>}
                      </svg>
                      <span style={{ fontSize:13, fontWeight:600, color:form.method===key?"#111":"#6B7280" }}>{m.label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div style={{ padding:"12px 14px", background:"rgba(245,158,11,0.06)", borderRadius:8, marginBottom:14, fontSize:12, color:"#6B7280", lineHeight:1.6, border:"1px solid rgba(245,158,11,0.2)" }}>
                By submitting, I confirm that account details are accurate. Beme Market is not liable for payments to incorrect accounts.
              </div>
              ...
              <div style={{ display:"flex", gap:10 }}>
                <button type="button" onClick={() => setShowForm(false)} style={{ flex:1, height:44, borderRadius:10, border:"1.5px solid rgba(0,0,0,0.12)", background:"transparent", color:"#333", fontSize:14, fontWeight:700, cursor:"pointer" }}>Cancel</button>
                <button type="submit" disabled={submitting} style={{ flex:2, height:44, borderRadius:10, border:"none", background:"#111", color:"#fff", fontSize:14, fontWeight:800, cursor:"pointer", opacity:submitting?0.7:1 }}>
