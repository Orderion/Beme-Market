...
const CATEGORY_KEYWORDS = [
  {
    label: "Iphones", type: "category", value: "iphone",
    aliases: ["phone","phones","iphone","android","mobile","smartphone","tecno","infinix","samsung","itel","pixel","ipad","tablet"],
  },
  {
    label: "Laptops", type: "category", value: "laptop",
    aliases: ["laptop","laptops","macbook","notebook","computer","pc","dell","hp","lenovo","acer","asus"],
  },
  {
    label: "Shoes", type: "category", value: "shoes",
    aliases: ["shoe","shoes","sneaker","sneakers","slides","sandals","heels","boots","slippers","airforce","air force"],
  },
  {
    label: "Clothing", type: "category", value: "clothing",
    aliases: ["clothing","clothes","fashion","shirt","shirts","dress","dresses","hoodie","hoodies","trousers","jeans","top","tops"],
  },
  {
    label: "Kids", type: "category", value: "kids",
    aliases: ["kids","kid","children","child","baby","babies","toddler","infant"],
  },
  {
    label: "Accessories", type: "category", value: "accessories",
    aliases: ["accessories","accessory","watch","bag","bags","power bank","speaker","perfume","cosmetics","others"],
  },
];
...
function CategoryIcon({ type }) {
  ...
  if (type === "shoes") return (
    <svg viewBox="0 0 24 24" className="category-box-svg" xmlns="http://www.w3.org/2000/svg">
      <path d="M4 15.5c2.2 0 3.3-1.2 4.4-2.4l1.2-1.3c.4 1.8 1.6 3.1 4 3.7l3 .8c1.8.5 2.4 1 2.4 2.2V20H4v-4.5Z" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
    </svg>
  );
  if (type === "clothing") return (
    <svg viewBox="0 0 24 24" className="category-box-svg" xmlns="http://www.w3.org/2000/svg">
      <path d="M9 4l3 2 3-2 3 3-2 3v9H8v-9L6 7l3-3Z" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
    </svg>
  );
  if (type === "kids") return (
    <svg viewBox="0 0 24 24" className="category-box-svg" xmlns="http://www.w3.org/2000/svg">
      <circle cx="12" cy="8" r="3" fill="none" stroke="currentColor" strokeWidth="1.7" />
      <path d="M7.5 19v-2.2A3.8 3.8 0 0 1 11.3 13h1.4a3.8 3.8 0 0 1 3.8 3.8V19" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
    </svg>
  );
  return (
    <svg viewBox="0 0 24 24" className="category-box-svg" xmlns="http://www.w3.org/2000/svg">
...
function CategoryQuickCard({ item, onClick }) {
  ...
  return (
    <div className="category-box" role="link" tabIndex={0} onClick={onClick} onKeyDown={handleKeyDown} aria-label={`Open ${item.label}`}>
      <div className="category-box-icon" aria-hidden="true"><CategoryIcon type={item.key} /></div>
...
export default function Home() {
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setActiveIndex((prev) => (prev < suggestions.length - 1 ? prev + 1 : 0)); }
    if (e.key === "ArrowUp") { e.preventDefault(); setActiveIndex((prev) => (prev > 0 ? prev - 1 : suggestions.length - 1)); }
    ...
    <div className="home">
      ...
      <div className="search-wrap" ref={searchWrapRef}>
        <form className="search-container" onSubmit={submitSearch}>
          ...
          <button type="submit" className="search-submit" aria-label="Search">
            <svg viewBox="0 0 24 24" className="search-filter-icon" xmlns="http://www.w3.org/2000/svg">
            ...
            {loadingSuggestions ? (
              ...
              suggestions.map((item, index) => (
                <button
                  key={item.id}
                  type="button"
                  className={index === activeIndex ? "search-suggestion-item active" : "search-suggestion-item"}
              ...
              <div className="search-suggestion-empty">No matching keywords found.</div>
            )}
          </div>
        ) : null}
      </div>
      ...
      <section className="section section--categories">
        <div className="category-grid">
          {CATEGORY_CARDS.map((item) => (
            <CategoryQuickCard key={item.key} item={item} onClick={() => goToCategory(item)} />
          ))}
        </div>
      </section>

      {/* ── Shops Carousel (replaces old store-grid) ── */}
      <section className="section">
        <div className="section-header">
          <h3>What's new on Beme Market</h3>
          <button className="see-all-btn" onClick={goToShop}>View all</button>
        </div>
      ...
      <section className="section">
        <div className="section-header">
          ...
          <button className="see-all-btn" onClick={() => navigate("/shop?featured=1")}>See featured</button>
        </div>
      ...
      <section className="section">
        <div className="section-header">
          ...
          <button className="see-all-btn" onClick={goToShop}>See all</button>
        </div>
