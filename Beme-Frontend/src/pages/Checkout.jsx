...
export default function Checkout() {
  ...
  /* Check order history — 55s timeout to handle Render cold-starts */
  useEffect(() => {
    let active = true;
    const check = async () => {
      if (authLoading) return;
      if (!user?.uid) { if(active){setHasPaidOrder(false);setCheckingHistory(false);} return; }
      setCheckingHistory(true);
      try {
        const data = await getMyOrders();
        if (!active) return;
        const rows = Array.isArray(data?.orders) ? data.orders : [];
        setHasPaidOrder(rows.some(isOrderPaid));
      } catch(e) {
        console.error("Failed to check order history:", e);
        if (!active) return;
        // On timeout/error, don't block checkout — assume eligible
  ...
  const needsFirstPaystack    = !checkingHistory && !hasPaidOrder;
  ...
  const buildDeliveryPayload = () => {
    ...
    // Normalize type to what server expects: "courier" | "self" | "beme_self"
    let deliveryType;
    if (isSellerDirect) {
      deliveryType = "self"; // seller arranges it
    } else if (provider) {
      deliveryType = "courier"; // third-party courier
    } else {
      deliveryType = "self";
    }

    return {
      type:     deliveryType,          // ← what backend validates
      method:   delivery.method,       // raw selection (cheetah / seller_direct / etc)
      ...
      breakdown:{ courierFee:courierFeeUI, abroadFee:abroadFeeUI, sellerArranged:isSellerDirect },
    };
  };
  ...
  const placeCOD = async () => {
    if(loading||isCODBlocked||checkingHistory||hasUnavailable)return;
  ...
  const payWithPaystack = async () => {
    setPaystackError("");
    if(loading||checkingHistory||hasUnavailable||!user)return;
  ...
  const isCheckoutDisabled = inputsDisabled||!!errors.cart||!user||authLoading||checkingHistory||hasUnavailable||(method==="cod"&&isCODBlocked);
              ...
              <div className="co-section">
                ...
                <div className="co-pay-methods">
                  ...
                  <label className={`co-pay-card${method==="paystack"?" co-pay-card--active":""}`}>
                    <input type="radio" name="payMethod" value="paystack" checked={method==="paystack"} onChange={handleMethodChange} disabled={inputsDisabled||checkingHistory} className="co-pay-radio"/>
                ...
                {checkingHistory&&<p className="co-hint">Checking eligibility…</p>}
