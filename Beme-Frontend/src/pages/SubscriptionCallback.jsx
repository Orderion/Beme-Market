...
export default function SubscriptionCallback() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ── Credit referral if a code was used during onboarding ──
        try {
          const referralCode = sessionStorage.getItem("beme_referral_code");
          if (referralCode && data.planId && data.planId !== "basic") {
            const auth = getAuth();
            const user = auth.currentUser;
            if (user) {
              await creditReferral({
                referrerCode:   referralCode,
                newSellerUid:   user.uid,
                newSellerEmail: user.email || "",
                planId:         data.planId,
              });
              // Clear so it doesn't fire again
              sessionStorage.removeItem("beme_referral_code");
            }
          }
        } catch (refErr) {
          // Referral credit failing should never block the subscription success
          console.warn("[referral credit]", refErr.message);
        }
  ...
  return (
    <div style={{
      ...
      background: "#f5f7fa", fontFamily: "var(--font-main,'DM Sans',sans-serif)", padding: 24,
    }}>
      <div style={{
        ...
        {status === "verifying" && (
          <>
            <div style={{ width:56, height:56, borderRadius:"50%", background:"rgba(4,110,242,0.1)",
              display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 20px" }}>
              <div style={{ width:28, height:28, border:"3px solid rgba(4,110,242,0.2)",
                borderTopColor:"#046EF2", borderRadius:"50%", animation:"spin 0.9s linear infinite" }}/>
            </div>
            <div style={{ fontSize:20, fontWeight:800, color:"#111", marginBottom:8 }}>Verifying payment…</div>
        ...
        {status === "success" && (
          <>
            ...
            <div style={{ fontSize:20, fontWeight:800, color:"#111", marginBottom:8 }}>Payment confirmed!</div>
            ...
            <div style={{ fontSize:13, color:"#9ca3af", fontWeight:500 }}>Redirecting to your dashboard…</div>
          </>
        )}
